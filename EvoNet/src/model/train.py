...
def augment(x, generation):
    noise_level = 0.1 * (1 - generation / number_of_generation)
...
def decode_chromosome(chromosome):
    ...
    w1_size = input_layer_size * hidden_layer_size
    weights_input_hidden = chromosome[index : index + w1_size].reshape(
        input_layer_size, hidden_layer_size
    )
    index += w1_size

    w2_size = hidden_layer_size * output_layer_size
    weights_hidden_output = chromosome[index : index + w2_size].reshape(
        hidden_layer_size, output_layer_size
    )
    index += w2_size
...
def decode_population(population):
    ...
    w2 = population[:, w1_size : w1_size + w2_size].reshape(
        pop_size, hidden_layer_size, output_layer_size
    )
...
def evaluate_population(population, generation):
    ...
    augmented_x = augment(batch_x.copy(), generation)
    ...
    train_probs = torch.softmax(logits_train, dim=2)
    val_probs = torch.softmax(logits_val, dim=2)

    train_correct = train_probs.gather(
        2, y_train_t.unsqueeze(0).unsqueeze(2).expand(pop_size, -1, 1)
    ).squeeze(2)
    val_correct = val_probs.gather(
        2, y_val_t.unsqueeze(0).unsqueeze(2).expand(pop_size, -1, 1)
    ).squeeze(2)

    train_acc = train_correct.mean(dim=1)
    val_acc = val_correct.mean(dim=1)

    fitness_scores = []

    for i in range(pop_size):
        pred = torch.argmax(train_probs[i], dim=1).cpu().numpy()
        unique, counts = np.unique(pred, return_counts=True)
        distribution_penalty = np.std(counts) / len(pred)

        penalty = 0.00001 * torch.mean(population[i] ** 2).item()

        confidence_bonus = train_correct[i].mean().item()
...
def tournament_selection(population, fitness_scores):
    ...
    ranks = np.argsort(np.argsort(selected_fitness))
    probs = (ranks + 1) / np.sum(ranks + 1)

    chosen = np.random.choice(selected_indices, p=probs)
...
def crossover(parent_one, parent_two):
    ...
    child = alpha * parent_one + (1 - alpha) * parent_two
    return child


def mutate(chromosome, generation):
    scale = 0.1 * (1 - generation / number_of_generation)
    chromosome += np.random.randn(*chromosome.shape) * scale
    return chromosome
...
for generation in range(number_of_generation):
    ...
    if true_best_chromosome is not None:
        new_population[0] = true_best_chromosome.clone()

    pop_np = population.cpu().numpy()

    while len(new_population) < population_size:

        parent_one = tournament_selection(pop_np, fitness_scores)
        parent_two = tournament_selection(pop_np, fitness_scores)
